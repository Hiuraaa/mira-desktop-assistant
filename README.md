...
5. Mira mở **Studio 3D** trong trình duyệt của chính máy tính khi khởi động. Giữ cửa sổ ứng dụng Mira đang chạy vì cửa sổ này xử lý chat và duyệt các quyền. Để mở lại Studio, bấm **Mở Studio 3D** trong Mira. Cửa sổ Tk cũ vẫn có lịch nhắc, cấu hình Ollama, Telegram và các quyền liên quan.

## Studio 3D, model VRM và bài học game

- **Nhân vật 3D ngay sau khi mở:** Studio có nhân vật anime 3D đơn giản được dựng tại máy. Bấm **Chọn VRM của bạn** để dùng model do bạn tạo hoặc một model có quyền sử dụng phù hợp. Studio nhận **VRM 0.x/1.0 tự chứa, dưới 20 MiB**, giữ tại `%LOCALAPPDATA%\Mira\mira_character.vrm`. Kéo chuột để xoay và lăn để phóng; phím **Xoay** đổi mặt. Phần mềm chỉ dùng WebGL của trình duyệt để dựng model, không có tải xuống hoặc phí thuê máy chủ 3D. Nếu VRM quá nặng, hãy giảm dung lượng khi xuất model và dùng nhân vật 3D mặc định trên máy yếu. Vật liệu đặc thù MToon, vật lý tóc và biểu cảm của mọi model VRM chưa được bảo đảm giống phần mềm VRoid; Studio có chuyển động đầu, mắt hoặc miệng khi model có dữ liệu tương ứng.
- **Tìm model miễn phí:** Tạo bằng [VRoid Studio](https://vroid.com/en/studio), [VRoid Mobile](https://vroid.com/en/mobile) hoặc dùng [mẫu VRoid CC0](https://vroid.pixiv.help/hc/en-us/articles/4402614652569-Do-VRoid-Studio-s-sample-models-come-with-conditions-of-use). Một số nhân vật ở VRoid Hub có điều kiện sử dụng riêng; hãy xem điều kiện trước khi tải và gắn với Mira. Thư viện Three.js r180 đã được đưa sẵn vào ZIP theo giấy phép MIT tại `mira/studio_assets/vendor/LICENSE.three.txt`; Studio không chạy JavaScript từ CDN.
- **Dạy thao tác game:** Ở **Dạy Mira một thao tác**, đặt tên, thêm tối đa 10 bước **bấm/giữ phím** rồi lưu. Bài học lưu tại `%LOCALAPPDATA%\Mira\game_lessons.json`. Bật quyền thao tác trên cửa sổ PC, bấm **Chọn cửa sổ game (3 giây)** rồi chuyển trọng tâm sang game để Mira ghi nhận đúng cửa sổ. Bấm **Thử** hoặc nhắn `/play <tên bài học>`; Mira hiện toàn bộ phím và tên cửa sổ để bạn duyệt trước mỗi lần chạy. Nút **DỪNG THAO TÁC** ngắt chuỗi và thả phím đang giữ. AI cũng có thể đề nghị chạy bài học đã lưu khi bạn yêu cầu; vẫn phải duyệt. Bài học không chạy từ Telegram/điện thoại, không bấm nền hay ghi lại phím của bạn.
- **Nhìn và hướng dẫn từng bước:** Trong Studio, bấm **Màn hình**, duyệt trên PC, chờ hai giây để chuyển sang game, xem ảnh trước ở ô chat rồi bấm **Gửi**. Ảnh chỉ nằm tạm trong RAM, hết hạn sau hai phút, và chỉ được gửi cho Ollama khi bạn gửi tin. Cần model có khả năng hiểu ảnh. Mira có thể giải thích khung hình và đề nghị thao tác sau khi bạn duyệt; không có chế độ tự chơi game liên tục hoặc đọc màn hình nền. Nhiều game toàn màn hình hay có chống gian lận chặn Windows SendInput và/hoặc chụp màn hình.
- **Bộ nhớ riêng:** Thêm điều Mira cần nhớ ngay trong Studio. Trò chuyện ở Studio dùng cùng các cuộc hội thoại, sở thích và model Ollama với ứng dụng trên máy. Giọng đọc tùy chọn dùng giọng của trình duyệt; chất lượng giọng phụ thuộc Windows/trình duyệt và không cần dịch vụ TTS trả phí.

Studio chỉ phục vụ `127.0.0.1` qua một cổng ngẫu nhiên với khóa phiên riêng. URL Studio là phiên cục bộ trên **PC**, không phải đường dẫn Tailscale/điện thoại. Đóng ứng dụng Mira sẽ đóng phiên. Hãy dùng Chrome hoặc Edge có WebGL; máy yếu có thể giảm kích thước cửa sổ Studio.
...
Đây là một tính cách hội thoại lấy cảm hứng từ AI VTuber, không phải bản sao mô hình, giọng nói hay nhân vật của Neuro-sama. Cửa sổ Tk và trang điện thoại vẫn có avatar 2D/ảnh PNG riêng. **Studio 3D** trên PC nhận file VRM, dựng nhân vật có chuyển động cơ bản và hiển thị chat bên cạnh. Mira có thể thực hiện một chuỗi bấm/giữ phím game ngắn do bạn dạy sau mỗi lần duyệt; chưa tự chơi game liên tục, đọc màn hình nền hay nghe microphone. Các thao tác file trên desktop vẫn cần quyền và xác nhận như trước.
