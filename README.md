...
5. Mira mở cửa sổ chat trên máy tính. Để chat khi laptop tắt, chọn **☁ Điện thoại khi máy tắt** và làm theo hướng dẫn cài Mira cloud bên dưới. Để kết nối máy tính khi đang bật, chọn **◈ Kết nối khi máy bật**.

## Mira trên điện thoại khi laptop tắt

Bản cloud riêng ở thư mục `cloud/` cho bạn **chat, ghi chú sở thích và đặt lịch nhắc từ trình duyệt điện thoại**. Nếu kết nối một bot Telegram mới, Mira gửi tin nhắc dù bạn đã đóng trang hoặc tắt laptop. Dữ liệu chat, ghi chú và lịch cloud lưu riêng trong Cloudflare D1 của bạn; không tự đồng bộ với chat/lịch desktop. Model AI chạy trên Cloudflare Workers AI, gói Free **có hạn mức**, không dùng CPU/GPU laptop.
...
Mira không âm thầm theo dõi màn hình hoặc nghe microphone. Telegram không được xem trạng thái hoặc màn hình máy. Bot Telegram desktop và chat trên điện thoại Tailscale được tra cứu web khi bạn bật tính năng này. Trên trang điện thoại Tailscale, bạn có thể bật riêng quyền xem trạng thái PC, chụp từng ảnh màn hình hoặc đọc thư mục được chọn trong **Kết nối khi máy bật** trên máy tính. Chat điện thoại không điều khiển chuột/bàn phím, sửa file hay chạy lệnh. Các trang web, ảnh và file có thể chứa hướng dẫn sai cho AI; hãy xem kỹ nội dung trước khi gửi.
...
| Trò chuyện từ điện thoại khi laptop tắt | Bấm **☁ Điện thoại khi máy tắt**, cài cloud theo [hướng dẫn](cloud/README.md) và mở URL Mira trên điện thoại. |
| Đặt lịch nhắc | Trên Mira cloud mở **Lịch nhắc**, nhập việc cần làm và chọn giờ; lịch riêng của app desktop vẫn có thể dùng khi máy bật. |
| Nhận nhắc qua Telegram lúc máy tắt | Tạo bot **mới** bằng @BotFather, chạy `python setup_telegram.py` trong thư mục `cloud`. Lịch cloud sẽ gửi qua bot này. |
...
### Bản cloud khi laptop tắt: trình duyệt và Telegram

Dùng [Mira cloud](cloud/README.md) cho điện thoại và bot Telegram mới; server chạy độc lập với laptop. Nếu chưa cài bot cloud, lịch cloud lưu lại nhưng không thông báo khi bạn đóng trang.

### Bot Telegram cũ chạy trên desktop (chỉ khi laptop bật)
...
2. Trên máy tính, giải nén bản Mira mới, chạy Mira → **◈ Kết nối khi máy bật → Nhắc qua Telegram**. Dán token, bấm **Bật / tạo mã mới**, chờ dòng **Telegram sẵn sàng**. Nếu muốn tự kết nối sau khi mở Mira, đánh dấu lưu token trong hồ sơ Windows; nếu bỏ chọn, Mira chỉ giữ token trong phiên chạy và bạn phải nhập lại lần sau.
...
**Báo thức trong ứng dụng Đồng hồ:** Telegram bot không có quyền tạo báo thức hệ thống trên Android/iPhone. Để điện thoại báo khi máy tính tắt, dùng bot của Mira cloud hoặc nhập lịch `.ics` vào ứng dụng Lịch hỗ trợ thông báo; báo thức của ứng dụng Đồng hồ vẫn phải đặt thủ công. Một ứng dụng Android riêng được cấp quyền báo thức có thể tạo báo thức trên Android; Mira hiện chưa có ứng dụng điện thoại như vậy.
...
2. Trên máy tính mở Mira → **◈ Kết nối khi máy bật** → **Bật / tạo mã mới**. Ghi lại mã 8 chữ số, hiệu lực 5 phút và chỉ dùng một lần.
...
Tính cách hội thoại lấy cảm hứng từ AI VTuber, không sao chép model hoặc giọng nói của Neuro-sama. Desktop vẫn có nhân vật 2D/ảnh PNG. Mira cloud có giao diện nhẹ trên điện thoại. Bài học bấm phím đã lưu trên PC được giữ nguyên nhưng không chạy qua điện thoại hay khi máy tắt. Các thao tác file và PC vẫn cần quyền và xác nhận như trước.
...
- Giao diện điện thoại Tailscale khi laptop đang bật chỉ mở khi bạn bật trong Mira, lắng nghe `127.0.0.1:8765` và cần Tailscale Serve, mã ghép nối một lần, cookie phiên cùng xác minh CSRF. Nếu đã ghép nối, người dùng cầm điện thoại hoặc tài khoản có quyền đăng nhập thiết bị đó có thể xem chat điện thoại và lịch nhắc. Không chia sẻ mã, tài khoản Tailscale hoặc thiết bị đã ghép nối.
...
`cloud/worker.mjs` cùng `cloud/public/` là Mira cloud trên điện thoại, `cloud/README.md` hướng dẫn cài; `mira/gui.py` chứa giao diện desktop; `mira/telegram_bot.py` kết nối Bot API qua long polling; `mira/mobile_server.py` và `mira/phone.html` phục vụ giao diện điện thoại qua loopback; `mira/reminders.py` lưu lịch và xuất `.ics`; `mira/agent.py` gọi Ollama và giới hạn công cụ; `mira/workspace.py` kiểm tra đường dẫn, diff và sao lưu; `mira/storage.py` lưu dữ liệu cục bộ; `mira/preferences.py` chọn sở thích theo ngữ cảnh. Không yêu cầu thư viện Python ngoài standard library cho tính năng hiện có.
